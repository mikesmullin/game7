...
BTStatus WaitTick(WaitNode* node) {
  if (g_engine->currentTime - node->startTime >= node->waitTime) {
    return BT_SUCCESS;
  }
...
BTStatus RandomTick(RandomNode* node) {
  ...
  // Pick a random child
  u32 randomIndex = Math__urandom2(0, node->childCount);
...
BTStatus PriorityTick(PriorityNode* node) {
  ...
  if (g_engine->currentTime - node->lastExecutionTime < node->period) {
    return BT_RUNNING;  // Not enough time has passed, stay in running
  }

  node->lastExecutionTime = g_engine->currentTime;  // Update last execution time
