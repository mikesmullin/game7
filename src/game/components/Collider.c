  ...
  for (u32 i = 0; i < matchCount; i++) {
    ...
    if (NULL != other->collider) {
      ...
      if (collisionBefore || collisionAfter) {
        ...
        OnCollideClosure params = {entity, other, x, y, collisionBefore, collisionAfter, false};
        Dispatcher__collide(other->collider->collide, other, state, &params);  // notify target
        if (params.noclip) return false;
        Dispatcher__collide(entity->collider->collide, entity, state, &params);  // notify source
        if (params.noclip) return false;
      }
